    ...
    const onSubmitHandler=(event)=>{
        ...
        return (
            <div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="form-group">{/*use form dan gelen handlesubmit state i on submit fonksiyonuna gönderiliyor */}
                    
                <label>id:{}</label>
                    <input name="id"  type="text" className="form-control col-md-5 " ref={register}></input>
                    <label>CategoryName:{}</label> 
                    <input name="categoryName"  type="text" className="form-control col-md-5 " ref={register} pattern="\d*" ></input>{/*pattern olayı ile yazılan metinler kontrol ediliyor */}
                    <label>SeoUrl{}</label>
                    <input name="city"  type="text" className="form-control col-md-5 " ref={register({required:"BOŞ BIRAKILAMAZ",minLength:{value:3, message:"şehir üç karakterden az olamaz"}})}></input>{/*register ile error mesajları kontrol edilebiliyor */}
